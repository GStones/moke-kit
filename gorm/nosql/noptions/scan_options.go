...
func WithLimit(limit int) ScanOption {
	return func(o *ScanOptions) error {
		if limit < 0 {
			return errors.Wrap(nerrors.ErrInternal, "negative limit specified")
		} else {
			o.Limit = limit
		}
		return nil
...
func WithOffset(offset int) ScanOption {
	return func(o *ScanOptions) error {
		if offset < 0 {
			return errors.Wrap(nerrors.ErrInternal, "negative offset specified")
		} else {
			o.Offset = offset
		}
		return nil
...
func MatchKeyLike(idx string, val string) ScanOption {
	return func(o *ScanOptions) error {
		if idx == "" {
			...
			return errors.Wrap(nerrors.ErrInvalidScanValue, "wildcard query string is too short")
		} else {
...
func MatchRegex(regex string) ScanOption {
	return func(o *ScanOptions) error {
		if regex == "" {
			return errors.Wrap(nerrors.ErrInternal, "empty regex provided")
		} else {
			o.Query = append(o.Query, ScanQuery{
